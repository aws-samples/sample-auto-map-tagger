...
ui_instr_single_title:'HƯỚNG DẪN TRIỂN KHAI — Một tài khoản',
            ...
            ui_stackset_bucket_note:'Quá trình triển khai này tạo một S3 bucket tạm thời (auto-map-tagger-{account-id}-{region}) để lưu trữ template CloudFormation cho StackSet. Bucket được giữ lại sau khi triển khai vì StackSet cần nó để triển khai đến các tài khoản mới tham gia tổ chức trong tương lai.',
            warn_no_us_east_1_global:'⚠️ us-east-1 không có trong các khu vực bạn đã chọn. Các dịch vụ toàn cầu (CloudFront, Route 53, Global Accelerator, Network Firewall, Direct Connect, App Mesh, Cloud Map) chỉ phát sự kiện CloudTrail ở us-east-1 và SʼẤʼ KHÔNG được gán thẻ trừ khi us-east-1 được bao gồm. Hãy thêm us-east-1 nếu bạn có tài nguyên dịch vụ toàn cầu cần gán thẻ để nhận tín dụng MAP.',
            ...
            ui_prereq_title:'Điều kiện tiên quyết', ui_prereq_subtitle:'deploy.sh được tạo sẽ tự động xác minh trước khi triển khai:',
            ...
            ui_scp_intro:'Nếu tổ chức của bạn sử dụng SCP, hãy xác minh những điều sau trước khi triển khai:',
            ui_deploy_instructions:'Hướng dẫn triển khai',
            ...
            ui_scp_mandatory:'SCP gắn tag bắt buộc — nếu SCP yêu cầu tag phải có khi tạo resource, giải pháp này sẽ không đáp ứng yêu cầu đó (tag được áp dụng 60-90 giây sau khi tạo). Hãy miễn trừ tag key map-migrated hoặc sử dụng khoảng thời gian ân hạn.',
            ui_iac_title:'🏗️ Đang dùng Terraform / IaC?',
            ui_iac_check_label:'Resource trong môi trường này được quản lý bằng Terraform (hoặc IaC khác)',
            ui_iac_reminder_title:'🏗️ Nhắc nhở — bản triển khai này được đánh dấu là môi trường quản lý bằng Terraform / IaC',
            ui_iac_drift:'Việc đồng bộ default_tags/tags_all của Terraform sẽ âm thầm xóa tag map-migrated ở lần apply tiếp theo (tag không được khai báo trong IaC sẽ bị gỡ). Thêm ignore_tags { keys = ["map-migrated"] } vào khối Terraform AWS provider của bạn, hoặc khai báo tag này trong IaC của bạn. CloudFormation thường giữ nguyên các tag được gắn ngoài luồng. Trình gắn tag phát hiện việc xóa tag ngoài luồng và phát cảnh báo CloudWatch kèm cách khắc phục này (chỉ cảnh báo — không bao giờ tự động gắn lại tag).',
            ...
            err_mpe_derived_length:"Với các khu vực và tùy chọn đã chọn, MAP ID chỉ được tối đa {max} ký tự — tên vai trò AWS IAM tạo từ ID này bị giới hạn 64 ký tự",
            ...
            ui_update_changeset_note:'Trước khi áp dụng bất kỳ thay đổi nào, upgrade.sh tạo một change-set CloudFormation và hiển thị chính xác những tài nguyên nào sẽ thay đổi (mã Lambda, IAM, EventBridge) — cấu hình phạm vi của bạn được giữ nguyên, không bị đặt lại. Bạn xác nhận trước khi nó thực thi. Để dùng không tương tác (CloudShell batch), chạy với --auto-approve. StackSet hiển thị bản tóm tắt dry-run thay thế, vì change-set không được hỗ trợ cho StackSet.',
