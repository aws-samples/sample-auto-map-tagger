...
ui_instr_single_title:'部署说明 — 单账户',
            ...
            ui_stackset_bucket_note:'此部署将创建一个临时S3存储桶（auto-map-tagger-{account-id}-{region}）来暂存StackSet的CloudFormation模板。由于StackSet需要该存储桶向未来加入组织的新账户进行部署，因此部署后将保留该存储桶。',
            warn_no_us_east_1_global:'⚠️ 您选择的区域中没有us-east-1。全球性服务（CloudFront、Route 53、Global Accelerator、Network Firewall、Direct Connect、App Mesh、Cloud Map）仅在us-east-1中生成CloudTrail事件，因此除非包含us-east-1，否则将不会被标记。如果您有需要为MAP贷金标记的全球性服务资源，请添加us-east-1。',
            ...
            ui_prereq_title:'前提条件', ui_prereq_subtitle:'生成的deploy.sh将在部署前自动验证：',
            ...
            ui_scp_intro:'如果您的组织使用了SCP，请在部署前验证以下内容：',
            ui_deploy_instructions:'部署说明',
            ...
            ui_scp_mandatory:'强制标签SCP — 如果SCP要求在资源创建时必须存在标签，此解决方案将无法满足该要求（标签在创建后60-90秒应用）。请豁免map-migrated标签键或设置宽限期。',
            ui_iac_title:'🏗️ 正在使用Terraform / IaC？',
            ui_iac_check_label:'此环境的资源使用Terraform（或其他IaC）管理',
            ui_iac_reminder_title:'🏗️ 提醒 — 此部署已标记为Terraform / IaC管理环境',
            ui_iac_drift:'Terraform的default_tags/tags_all协调会在下次apply时静默删除map-migrated标签（未在IaC中声明的标签会被清除）。请在您的Terraform AWS provider块中添加 ignore_tags { keys = ["map-migrated"] }，或在您的IaC中声明该标签。CloudFormation通常会保留带外应用的标签。标记器会检测带外标签删除并触发包含此修复方法的CloudWatch告警（仅告警 — 绝不自动重新标记）。',
            ...
            err_mpe_derived_length:"根据所选区域和选项，MAP参与ID最多为{max}个字符 — 由其派生的AWS IAM角色名称限制为64个字符",
            ...
            ui_update_changeset_note:'在应用任何更改之前，upgrade.sh 会创建一个 CloudFormation 变更集（change-set），并准确显示哪些资源将发生变化（Lambda 代码、IAM、EventBridge）——您的范围配置将被保留而不会重置。执行前会要求确认。非交互式（CloudShell 批处理）使用时，请加 --auto-approve 运行。StackSet 不支持变更集，因此改为显示试运行摘要。',
