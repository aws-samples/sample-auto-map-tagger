...
1. Generate a customer-specific `deploy.sh` from `build/configurator.html` (client-side; runs fully in the browser and requires no outbound network access).
...
`build/configurator.html` is checked into this repo. Open it in any modern browser — no server, no installation, no network call required.
