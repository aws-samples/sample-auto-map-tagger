...
ui_instr_single_title:'デプロイ手順 — シングルアカウント',
            ...
            ui_stackset_bucket_note:'このデプロイメントは、StackSet用のCloudFormationテンプレートをステージするために一時的なS3バケット（auto-map-tagger-{account-id}-{region}）を作成します。StackSetが将来組織に追加される新しいアカウントへのデプロイに使用するため、バケットはデプロイ後も保持されます。',
            warn_no_us_east_1_global:'⚠️ 選択したリージョンにus-east-1がありません。グローバルサービス（CloudFront、Route 53、Global Accelerator、Network Firewall、Direct Connect、App Mesh、Cloud Map）はus-east-1でのみCloudTrailイベントを発行するため、us-east-1を含めないとタグ付けされません。グローバルサービスのリソースをMAPクレジット用にタグ付けする場合はus-east-1を追加してください。',
            ...
            ui_prereq_title:'前提条件', ui_prereq_subtitle:'生成されたdeploy.shがデプロイ前に自動的に確認します:',
            ...
            ui_scp_intro:'組織でSCPを使用している場合は、デプロイ前に以下を確認してください:',
            ui_deploy_instructions:'デプロイ手順',
            ...
            ui_scp_mandatory:'必須タグSCP — SCPがリソース作成時にタグの存在を要求する場合、このソリューションはその要件を満たしません（タグは作成後60〜90秒後に適用）。map-migratedタグキーを作成時要件から除外するか、猶予期間を設定してください。',
            ui_iac_title:'🏗️ Terraform / IaCをご利用ですか？',
            ui_iac_check_label:'この環境のリソースをTerraform（または他のIaC）で管理している',
            ui_iac_reminder_title:'🏗️ リマインダー — このデプロイはTerraform / IaC管理環境としてマークされています',
            ui_iac_drift:'Terraformのdefault_tags/tags_allの調整により、次回のapply時にmap-migratedタグが暗黙的に削除されます（IaCに宣言されていないタグは除去されます）。Terraform AWSプロバイダーブロックに ignore_tags { keys = ["map-migrated"] } を追加するか、IaC内でタグを宣言してください。CloudFormationは通常、外部で適用されたタグを保持します。タガーは外部からのタグ削除を検出し、この修正方法を含むCloudWatchアラームを発報します（アラートのみ — 自動再タグ付けは行いません）。',
            ...
            err_mpe_derived_length:"選択したリージョンとオプションでは、MAP IDは最大{max}文字までです — このIDから生成されるAWS IAMロール名が64文字に制限されているためです",
            ...
            ui_update_changeset_note:'適用前に upgrade.sh が CloudFormation の変更セット（change-set）を作成し、どのリソースが変更されるか（Lambda コード、IAM、EventBridge）を正確に表示します。スコープ設定はリセットされず保持されます。実行前に確認を求めます。非対話型（CloudShell バッチ）で使用する場合は --auto-approve を付けて実行してください。StackSet は change-set をサポートしないため、代わりにドライラン概要を表示します。',
