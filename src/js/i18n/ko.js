...
ui_instr_single_title:'배포 지침 — 단일 계정',
            ...
            ui_iac_check_label:'고객이 Terraform(또는 기타 IaC)으로 리소스를 관리합니다',
            ui_iac_reminder_title:'🏗️ 리마인더 — 이 고객을 Terraform / IaC 사용으로 표시하셨습니다',
            ui_iac_drift:'Terraform의 default_tags/tags_all 재조정은 다음 apply 시 map-migrated 태그를 조용히 제거합니다 (IaC에 선언되지 않은 태그는 제거됨). 고객이 Terraform AWS provider 블록에 ignore_tags { keys = ["map-migrated"] }를 추가하거나 IaC에 해당 태그를 선언하도록 안내하세요. CloudFormation은 일반적으로 외부에서 적용된 태그를 유지합니다. 태거는 외부 태그 제거를 감지하여 이 해결 방법이 포함된 CloudWatch 경보를 발생시킵니다 (경보 전용 — 자동으로 다시 태그하지 않음).',
