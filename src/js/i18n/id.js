...
ui_instr_single_title:'INSTRUKSI DEPLOYMENT — Satu Akun',
            ...
            ui_stackset_bucket_note:'Deployment ini membuat bucket S3 sementara (auto-map-tagger-{account-id}-{region}) untuk menyimpan template CloudFormation bagi StackSet. Bucket dipertahankan setelah deployment karena StackSet memerlukannya untuk men-deploy ke akun baru yang bergabung dengan organisasi Anda di masa mendatang.',
            warn_no_us_east_1_global:'⚠️ us-east-1 tidak ada di region yang Anda pilih. Layanan global (CloudFront, Route 53, Global Accelerator, Network Firewall, Direct Connect, App Mesh, Cloud Map) hanya menghasilkan event CloudTrail di us-east-1 dan TIDAK akan diberi tag kecuali us-east-1 disertakan. Tambahkan us-east-1 jika Anda memiliki resource layanan global untuk diberi tag demi kredit MAP.',
            ...
            ui_prereq_title:'Prasyarat', ui_prereq_subtitle:'deploy.sh yang dihasilkan akan memverifikasi ini secara otomatis sebelum deployment:',
            ...
            ui_scp_intro:'Jika organisasi Anda menggunakan SCP, verifikasi hal berikut sebelum deployment:',
            ui_deploy_instructions:'Instruksi Deployment',
            ...
            ui_scp_mandatory:'SCP Tagging Wajib — jika SCP mengharuskan tag ada saat resource dibuat, solusi ini tidak akan memenuhi persyaratan tersebut (tag diterapkan 60-90 detik setelah pembuatan). Kecualikan kunci tag map-migrated atau gunakan masa tenggang.',
            ui_iac_title:'🏗️ Menggunakan Terraform / IaC?',
            ui_iac_check_label:'Resource di lingkungan ini dikelola dengan Terraform (atau IaC lain)',
            ui_iac_reminder_title:'🏗️ Pengingat — deployment ini ditandai sebagai lingkungan yang dikelola Terraform / IaC',
            ui_iac_drift:'Rekonsiliasi default_tags/tags_all Terraform secara diam-diam menghapus tag map-migrated pada apply berikutnya (tag yang tidak dideklarasikan dalam IaC akan dihapus). Tambahkan ignore_tags { keys = ["map-migrated"] } ke blok Terraform AWS provider Anda, atau deklarasikan tag tersebut dalam IaC Anda. CloudFormation umumnya membiarkan tag yang diterapkan di luar IaC. Tagger mendeteksi penghapusan tag di luar IaC dan memicu alarm CloudWatch berisi perbaikan ini (hanya peringatan — tidak pernah menandai ulang secara otomatis).',
            ...
            err_mpe_invalid:"MAP ID harus 1–44 karakter, hanya huruf besar Inggris (A–Z) dan angka (0–9), serta minimal satu huruf dan satu angka",
            err_mpe_derived_length:"Untuk region dan opsi yang dipilih, MAP ID maksimal {max} karakter — nama peran AWS IAM yang diturunkan darinya dibatasi 64 karakter",
            ...
            ui_update_changeset_note:'Sebelum menerapkan apa pun, upgrade.sh membuat change-set CloudFormation dan menampilkan dengan tepat sumber daya mana yang akan berubah (kode Lambda, IAM, EventBridge) — konfigurasi scope Anda dipertahankan, tidak direset. Anda mengonfirmasi sebelum dijalankan. Untuk penggunaan non-interaktif (batch CloudShell), jalankan dengan --auto-approve. StackSet menampilkan ringkasan dry-run sebagai gantinya, karena change-set CloudFormation tidak didukung untuk StackSet.',
