...
ui_instr_single_title:'部署说明 — 单账户',
            ...
            ui_iac_check_label:'客户使用Terraform（或其他IaC）管理资源',
            ui_iac_reminder_title:'🏗️ 提醒 — 您已将此客户标记为使用Terraform / IaC',
            ui_iac_drift:'Terraform的default_tags/tags_all协调会在下次apply时静默删除map-migrated标签（未在IaC中声明的标签会被清除）。请让客户在Terraform AWS provider块中添加 ignore_tags { keys = ["map-migrated"] }，或在其IaC中声明该标签。CloudFormation通常会保留带外应用的标签。标记器会检测带外标签删除并触发包含此修复方法的CloudWatch告警（仅告警 — 绝不自动重新标记）。',
