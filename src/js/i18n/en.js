const en_translations = {
            ...
            ui_prereq_title:'Prerequisites', ui_prereq_subtitle:'the generated deploy.sh verifies these automatically before deploying:',
            ...
            ui_scp_intro:'If your organization uses SCPs, verify the following before deployment:',
            ui_deploy_instructions:'Deployment Instructions',
            ...
            ui_scp_mandatory:"Mandatory tagging SCPs — if SCPs require tags at resource creation time, this solution does not satisfy that requirement (tags are applied 60-90 seconds after creation). Exempt the tag key map-migrated from creation-time enforcement, or use a grace period.",
            ui_iac_title:'🏗️ Using Terraform / IaC?',
            ui_iac_check_label:'Resources in this environment are managed with Terraform (or other IaC)',
            ui_iac_reminder_title:'🏗️ Reminder — this deployment was flagged as Terraform / IaC-managed',
            ui_iac_drift:"Terraform's default_tags/tags_all reconciliation silently removes the map-migrated tag on the next apply (tags not declared in the IaC are stripped). Add ignore_tags { keys = [\"map-migrated\"] } to your Terraform AWS provider block, or declare the tag in your IaC. CloudFormation generally leaves out-of-band tags alone. The tagger detects out-of-band removal and raises a CloudWatch alarm with this fix (alert-only — it never re-tags automatically).",
