...
def get_config():
    ...
    if _config is None or (now - _config_ts) > _CONFIG_TTL_SECONDS:
        try:
            ...
            # config_error marks this as "config broken", NOT "resource
            # out of scope" — _process_event routes those events to
            # transient so they retry and, if the config stays broken,
            # exhaust into the DLQ and fire the DLQ alarm instead of
            # being silently acked (CT6-005: a malformed day-2 SSM edit
            # used to silently lose every in-window tag with no signal).
            _config = None
            _config_ts = 0.0
            ...
            return {
                ...
                'config_error': f'config unreachable/unparseable: {str(e)[:200]}',
            }
...
def _is_wellformed_arn(arn):
    """Reject structurally invalid ARNs from CloudTrail resources arrays.

    The account field (parts[4]) must be empty (global services like S3)
    or a 12-digit account ID, and the resource part must not be the
    literal string 'null' (JSON null serialized into the ARN).
    """
    parts = arn.split(':', 5)
    if len(parts) != 6:
        return False
    account = parts[4]
    if account and not (account.isdigit() and len(account) == 12):
        return False
    resource = parts[5]
    if resource == 'null' or resource.endswith('/null') or resource.endswith(':null'):
        return False
    return True
...
def extract_arn(detail, account_id, region):
    ...
    # Validate shape before trusting: AWS occasionally ships malformed
    # resources-array ARNs. Live case (2026-06): Kinesis CreateStream
    # events gained a resources array with the stream name in the
    # account-ID field and literal 'null' as the resource name
    # (arn:aws:kinesis:<region>:<streamName>:stream/null) — taking it
    # verbatim made every Kinesis stream a silent tag loss. A malformed
    # entry falls through to the dedicated per-service handlers below.
    if resources and len(resources) > 0:
        for resource in resources:
            arn = resource.get('ARN')
            # Skip Glue catalog ARNs - we want the database/table ARN
            if arn and ':catalog' not in arn and _is_wellformed_arn(arn):
                return arn
...
def tag_resource(arn, tag_key, tag_value, tagging_client=None, ec2_client=None, target_region=None):
    ...
    if ':s3:::' in arn:
        ...
        return True
    # ':agent-alias/' must be listed even though ':agent/' looks similar —
    # the substring test is against the full ARN and alias ARNs are
    # arn:...:agent-alias/<agentId>/<aliasId> (no ':agent/' segment).
    # Without it, alias ARNs fell through to RGTA, which rejects them on
    # ARN-regex validation → alert + untagged (P27B-BEDROCK-AGENT-ALIAS,
    # live-traced 2026-07-16).
    elif ':bedrock:' in arn and any(r in arn for r in [':agent/',':agent-alias/',':flow/',':prompt/',':knowledge-base/']):
...
def _process_event(eb_event, config):
    ...
    (batchItemFailures).
    """
    # CT6-005: a broken config is NOT "resource out of scope". Acking
    # events while the config is unreachable/malformed silently loses
    # every in-window tag with zero operator signal (no DLQ, no alarm).
    # Classify as transient instead: SQS redelivers; if the customer
    # fixes the config within the retry budget the tags land, otherwise
    # the events exhaust into EventDLQ and the DLQ alarm fires — the
    # customer fixes the config and redrives the DLQ. Tagging stays
    # fail-closed throughout (nothing tags while the config is broken).
    def _config_broken():
        if config.get('config_error'):
            return config['config_error']
        if not config.get('mpe_id'):
            return 'mpe_id missing from config'
        asd = config.get('agreement_start_date')
        try:
            datetime.strptime(asd, '%Y-%m-%d')
        except (ValueError, TypeError):
            return f"invalid agreement_start_date: '{asd}'"
        return None

    broken = _config_broken()
    if broken:
        print(f'CONFIG_BROKEN (transient, will retry via SQS): {broken}')
        return ('transient', broken)
