...
### The AWS account team generates one file

The AWS account team opens `configurator.html`, fills in the MAP Engagement ID and details, and downloads a single `deploy.sh` script. Everything is self-contained inside it.
...
1. **Receive** `deploy.sh` from their AWS account team (generated by the configurator tool)
...
## For the AWS Account Team: How to Generate a Template
...
5. Send `deploy.sh` to the customer with instructions to run it in CloudShell

The configurator takes ~2 minutes to use and produces a ready-to-deploy script with the customer's MAP Engagement ID pre-filled.
