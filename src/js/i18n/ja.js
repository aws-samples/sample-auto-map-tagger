...
ui_instr_single_title:'デプロイ手順 — シングルアカウント',
            ...
            ui_iac_check_label:'お客様がTerraform（または他のIaC）でリソースを管理している',
            ui_iac_reminder_title:'🏗️ リマインダー — このお客様はTerraform / IaC利用としてマークされています',
            ui_iac_drift:'Terraformのdefault_tags/tags_allの調整により、次回のapply時にmap-migratedタグが暗黙的に削除されます（IaCに宣言されていないタグは除去されます）。お客様にTerraform AWSプロバイダーブロックへ ignore_tags { keys = ["map-migrated"] } を追加するか、IaC内でタグを宣言するようご案内ください。CloudFormationは通常、外部で適用されたタグを保持します。タガーは外部からのタグ削除を検出し、この修正方法を含むCloudWatchアラームを発報します（アラートのみ — 自動再タグ付けは行いません）。',
