...
**Fixed (v22.1.0 — broken day-2 config silently lost tags, CT6-005):**

- **A malformed or unreachable SSM config silently lost every in-window tag.** Scoping correctly fails closed when `/auto-map-tagger/<mpe>/config` is unreadable, unparseable, or day-2-edited into an invalid state (missing `mpe_id`, mangled `agreement_start_date`) — nothing tags, which is right. But the affected events were **acked as skipped**: no retry, no DLQ, no alarm — the customer's config typo cost them every tag until someone noticed by accident. Broken-config events now classify **transient**: SQS redelivers (a config fixed within the ~15-min retry budget loses nothing), and if the config stays broken the events exhaust into EventDLQ and the DLQ alarm fires — the operator signal that was missing. Fix the config, then redrive the DLQ to recover the tags. Tagging stays fail-closed throughout. Note: while a config is broken, *all* in-scope-window events accumulate in the DLQ by design — the alarm firing loudly is the point; drain by redriving after the fix. Genuinely out-of-scope / out-of-window events under a *healthy* config are still acked silently (no new DLQ noise).

**Fixed (v22.1.0 — create-race not-found misclassification, CT-suite parallel fleet run 2026-07-18):**

- **Resources whose tag call raced their own provisioning stayed permanently untagged.** Under burst load (100 DynamoDB tables created at once, gate check `P3-C11-DDB`), the tagger's `TagResource` fired seconds after `CreateTable` while tables were still `CREATING`; DynamoDB returned `ResourceNotFoundException: Requested resource not found`, which the classifier treated as permanent-ignorable ("resource deleted, skip silently") — no retry, no alert, tag permanently lost (7/100 tables in the run; confirmed live: tables ACTIVE, zero tags). The same not-found-while-provisioning shape exists for other services' not-found codes in the ignorable list (`DBInstanceNotFound`, `InvalidVolume.NotFound`, …). Not-found errors are now disambiguated **by event age**: within 10 minutes of the CloudTrail `eventTime` they classify transient (SQS redelivers; the retry lands once provisioning settles), past it they classify permanent-ignorable (genuinely deleted, ack silently). A missing/unparseable `eventTime` takes the safe branch (retry). The 10-min grace deliberately sits below the 15-min SQS receive lifetime (5 × 180s) so genuinely-deleted resources still age out and get acked *before* exhausting redelivery — deletions never reach the DLQ or fire the alarm. Regression-tested by executing the real classifier through the age matrix (`tests/unit/lambda-create-race.test.js`). Same bug class as `P1-ECACHE-CL` below; this closes the remaining not-found siblings.
...
**Fixed (v22.1.0 — org/StackSet CloudTrail blind spot, customer incident 2026-07-16):**

- **Multi-account (org/StackSet) deploys could silently deploy into an account with no CloudTrail coverage — zero tags, zero errors, forever.** `deploy.sh`'s Step 1 preflight ran `aws cloudtrail describe-trails` with the *caller's* credentials, i.e. only in the management account where the script runs. It never checked the StackSet *target* (linked) accounts, where the actual tagger Lambda + EventBridge rule get deployed. A linked account with no local trail and no org trail shadowing one in would get a fully "successful" CloudFormation deploy — Lambda, SQS, EventBridge rule, SSM config all created — but the EventBridge rule (which matches `AWS API Call via CloudTrail` events exclusively) would never receive a single event. No DLQ message, no alarm, no log entry: there's nothing to fail on if the trigger never fires. Reported live by a customer (3 linked accounts, 2 regions) after a clean delete + redeploy still tagged nothing in ap-southeast-1.
  - **Fix:** the existing `PreflightFunction` custom resource — already deployed into every target account (single-account stack or StackSet instance) and already gating `AutoTaggerFunction`'s creation via `DependsOn` — now also checks CloudTrail coverage locally, in that account and region, before the tagger Lambda is created. `describe_trails(includeShadowTrails=True)` + `get_trail_status` from inside the target account transparently sees a local trail, a multi-region trail homed elsewhere, or an AWS Organizations trail shadowed in from the management account — no org-trail special-casing needed, and no extra credentials required. If no trail is actively logging that region, stack creation fails immediately with a clear reason instead of silently succeeding.
  - **Scope:** the check runs on `Create` only (matching the existing peer-conflict preflight's upgrade-detection behavior) — it validates new deployments, not in-place updates to already-running taggers.
  - New `PreflightRole` IAM grant: `cloudtrail:DescribeTrails`, `cloudtrail:GetTrailStatus` (read-only, `Resource: '*'` — CloudTrail does not support resource-level scoping for these actions).
  - **Separately investigated in the same incident:** the customer also reported zero SNS emails, including the initial subscription-confirmation email. Traced the org deployer's `ensure_central_topics()` — the `sns.subscribe()` call there is unconditional on `alert_email` being set and runs at deploy time, independent of CloudTrail or any tagging activity; since the StackSet resources deployed successfully, this code path executed without error. This is **not the same bug** and is not fixed here — most likely explanation is an AWS-side email suppression/opt-out on that address (silently drops confirmation sends with no API-visible error). Flagged for the customer to check `aws sns list-subscriptions-by-topic` for a `PendingConfirmation` entry.
  - **`deploy.sh` now surfaces the actual failure reason for a failed StackSet instance**, not just a count. Previously "❌ N account(s) failed. Check CloudFormation StackSet for details." forced a separate console/CLI dig; it now queries `StatusReason` per failed/cancelled instance and prints it inline (e.g. the CloudTrail preflight's message above), per account and region.
  - **Regression tests:** `tests/unit/preflight-cloudtrail.test.js`, `tests/unit/preflight-cloudtrail-behavior.test.js`, `tests/unit/deploy-script.test.js`.
**Fixed (v22.1.0 — CT6 preflight gaps, chaos-test blockers CT6-006 + CT6-007):**

- **Long MPE IDs deployed fine from the UI, then died in CloudFormation with `ROLLBACK_COMPLETE` (CT6-006, S1).** #96 raised the MPE field's MaxLength 20→44 without auditing the names derived from it: `map-auto-tagger-backfill-mig<id>-<region>` overflows AWS's 64-char IAM RoleName limit region-dependently (ap-northeast-2 caps the user-entered ID at 21 chars with backfill on; us-east-1 at 26). The configurator now computes the exact cap for the selected regions/options (backfill on/off, org deploy role) and rejects the ID at validation time with the actual limit in the error message (all 7 locales), instead of letting the deploy fail mid-CloudFormation.
- **`deploy.sh`'s competing-tagger preflight was blind to StackSet-deployed peers (CT6-007, S2).** The preflight matched only `map-auto-tagger-*` stack names, but a tagger that arrives via StackSet AutoDeployment is named `StackSet-map-auto-tagger-<mpe>-<uuid>` — invisible to the check, so a second scope=ALL tagger could pass preflight and race the incumbent on every CloudTrail event (last-writer wins on the tag value). The preflight now also matches the `StackSet-` prefix and strips the instance-name UUID before the peer's SSM config lookup (the same strip the runtime peer detector needed in PR #85).
**Fixed (v22.1.0 — org migration silently deployed nothing, CT6-004 S0):**

- **The documented org-mode v21→v22 migration left ZERO taggers org-wide while printing "Setup Complete!".** Three compounding defects, all fixed:
  1. **Docs omission:** `INSTRUCTIONS.md`'s StackSet-removal steps deleted the stack instances and the StackSet but not the management-account admin stack (the parent that creates the StackSet); the adjacent "IAM roles are never touched" note conflated the admin *stack* with the admin *roles*, reinforcing the omission. The migration steps now include the admin-stack deletion and spell out the distinction.
  2. **Swallowed no-op update:** with the admin stack surviving, `deploy.sh`'s `update-stack` on an identical template returned "No updates are to be performed" — silenced by `>/dev/null 2>&1 || true`. CloudFormation only re-invokes the StackSet-creating custom resource on a property change, so the run deployed nothing. The no-op is now detected and **fails the deploy** with the exact delete-then-rerun remediation; other `update-stack` errors also fail instead of passing silently.
  3. **Zero-instance poll fall-through:** the StackSet wait needed `TOTAL>0` to declare success and `FAILED>0` to declare failure, so `TOTAL==0` (StackSet deleted, or targets empty) fell through to a catch-all that flipped it to SUCCESS after 1200s. Zero instances after the wait is now a loud failure naming the StackSet.
  - Validated workaround for stacks already in this state (unchanged): delete the leftover admin stack in the management account, re-run `deploy.sh`.
...
- **Standalone ECS/Fargate tasks were never tagged.** `RunTask` was subscribed (EventBridge rule + SQS delivery worked), but the Lambda had no RunTask extractor and the universal ARN scan cannot reach the task ARN — it lives in a **list** (`responseElements.tasks[].taskArn`), and the scan only walks top-level keys plus one-level dicts. `extract_arn` returned None → silent drop, no DLQ, no alert. Because Fargate billing attributes to *tasks* (not services), the already-working `CreateService` tagging never covered Fargate spend. Surfaced via a customer billing analysis (~$86–200/mo untagged Fargate spend). A dedicated multi-resource extractor now tags every task in the event (a single `RunTask --count N` call launches N tasks). Landed with a real captured CloudTrail fixture (golden-event test), per the #102 event-shape-drift lesson. **UNVERIFIED** pending live end-to-end tag observation — tracked in COVERAGE.md.
  - **Scope boundary (verified empirically):** ECS *service-scheduler* task launches do NOT emit a customer-visible `RunTask` CloudTrail event — a `desiredCount=1` service ran for 25+ minutes producing zero RunTask events. Service-launched Fargate tasks therefore cannot be tagged by any event-driven tagger; customers must set `--propagate-tags SERVICE` on their services. New LIMITATIONS.md section "Fargate tasks launched by ECS services need `propagateTags`".
- **New CI gate: subscription parity** (`audit_handler_coverage.py --parity`). The class behind the RunTask gap: an event subscribed in `src/js/services/*.js` with no `lambda-handler.py` branch was invisible to every existing check (the coverage audit only inventories explicit `event_name ==` branches). CI now fails on any subscribed event that has no dedicated extractor, is not in `_IGNORE_EVENTS`, and is not consciously baselined in `.github/subscription_parity_baseline.txt` as universal-scan-reliant (65 events baselined at introduction).
