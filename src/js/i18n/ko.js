...
ui_instr_single_title:'배포 지침 — 단일 계정',
            ...
            ui_stackset_bucket_note:'이 배포는 StackSet용 CloudFormation 템플릿을 준비하기 위해 임시 S3 버킷(auto-map-tagger-{account-id}-{region})을 생성합니다. StackSet이 향후 조직에 새로 추가되는 계정에 배포하기 위해 이 버킷이 필요하므로 배포 후에도 유지됩니다.',
            warn_no_us_east_1_global:'⚠️ 선택한 리전에 us-east-1이 없습니다. 글로벌 서비스(CloudFront, Route 53, Global Accelerator, Network Firewall, Direct Connect, App Mesh, Cloud Map)는 us-east-1에서만 CloudTrail 이벤트를 발생시키므로, us-east-1을 포함하지 않으면 태깅되지 않습니다. 글로벌 서비스 리소스를 MAP 크레딧용으로 태깅해야 한다면 us-east-1을 추가하세요.',
            ...
            ui_prereq_title:'사전 요구 사항', ui_prereq_subtitle:'생성된 deploy.sh가 배포 전 자동으로 확인합니다:',
            ...
            ui_scp_intro:'조직에서 SCP를 사용하는 경우 배포 전 다음 사항을 확인하세요:',
            ui_deploy_instructions:'배포 지침',
            ...
            ui_scp_mandatory:'필수 태그 SCP — SCP가 리소스 생성 시 태그를 요구하는 경우 이 솔루션은 해당 요구 사항을 충족하지 않습니다 (태그는 생성 후 60~90초 후에 적용됨). map-migrated 태그 키를 생성 시 적용 요건에서 제외하거나 유예 기간을 설정하세요.',
            ui_iac_title:'🏗️ Terraform / IaC를 사용 중이신가요?',
            ui_iac_check_label:'이 환경의 리소스를 Terraform(또는 기타 IaC)으로 관리합니다',
            ui_iac_reminder_title:'🏗️ 리마인더 — 이 배포는 Terraform / IaC 관리 환경으로 표시되었습니다',
            ui_iac_drift:'Terraform의 default_tags/tags_all 재조정은 다음 apply 시 map-migrated 태그를 조용히 제거합니다 (IaC에 선언되지 않은 태그는 제거됨). Terraform AWS provider 블록에 ignore_tags { keys = ["map-migrated"] }를 추가하거나 IaC에 해당 태그를 선언하세요. CloudFormation은 일반적으로 외부에서 적용된 태그를 유지합니다. 태거는 외부 태그 제거를 감지하여 이 해결 방법이 포함된 CloudWatch 경보를 발생시킵니다 (경보 전용 — 자동으로 다시 태그하지 않음).',
            ...
            err_mpe_derived_length:"선택한 리전과 옵션에서는 MAP 참여 ID가 최대 {max}자까지 가능합니다 — 이 ID로 생성되는 AWS IAM 역할 이름이 64자로 제한되기 때문입니다",
            ...
            ui_update_changeset_note:'적용 전에 upgrade.sh가 CloudFormation 변경 세트(change-set)를 생성하여 어떤 리소스가 변경되는지(Lambda 코드, IAM, EventBridge) 정확히 보여줍니다 — 스코프 설정은 초기화되지 않고 보존됩니다. 실행 전에 확인을 요청합니다. 비대화형(CloudShell 배치) 사용 시 --auto-approve로 실행하세요. StackSet은 change-set을 지원하지 않아 대신 드라이런 요약을 표시합니다.',
